dashboard "turbot_control_alarm_report_age" {
  title         = "Turbot Controls Alarm Age Report"
  ...
  container {
    ...
    card {
      query = query.turbot_control_alarm_total_count
      width = 2
      type  = "alert"
      label = "Count"
    }
    ...
    card {
      query = query.turbot_control_alert_24_hours_count
      width = 2
      type  = "alert"
      label = "< 24 hours"
    }
    ...
    card {
      query = query.turbot_control_alert_between_1_30_days
      width = 2
      type  = "alert"
      label = "1-30 Days"
    }
    ...
    card {
      query = query.turbot_control_alert_between_30_90_days
      width = 2
      type  = "alert"
      label = "30-90 Days"
    }
    ...
    card {
      query = query.turbot_control_alert_between_90_365_days
      width = 2
      type  = "alert"
      label = "90-365 Days"
    }
    ...
    card {
      query = query.turbot_control_alert_after_1_year
      width = 2
      type  = "alert"
      label = "> 1 Year"
    }
    ...
    table {
      title = "Controls"
      query = query.turbot_control_alert_oldest
...
query "turbot_control_alarm_total_count" {
  sql = <<-EOQ
    select
      count(*) as value,
      'Count' as label 
...
query "turbot_control_alert_24_hours_count" {
  sql = <<-EOQ
...
query "turbot_control_alert_between_1_30_days" {
  sql = <<-EOQ
...
query "turbot_control_alert_between_30_90_days" {
  sql = <<-EOQ
...
query "turbot_control_alert_between_90_365_days" {
  sql = <<-EOQ
...
query "turbot_control_alert_after_1_year" {
  sql = <<-EOQ
...
query "turbot_control_alert_oldest" {
  sql = <<-EOQ
    select
      id,
      workspace,
